...
$$L^p(P)=L^p(X)=\{X:X\to\mathbb{R}^n;\|X\|_p\lt\infty\}$$
...
$\sigma$-algebra $\mathcal{H}_{X_i}$들이 independent라는 것이다.

\ltdiv class="env-box box-definition">
...
Process $X=\{X_t\}_{t\in T}$의 (finite-dimensional) distribution at the
measure $\mu_{t_1,\cdots,t_k}$는 $\mathbb{R}^{nk}$위에서 정의되고
...
로 정의하자. $0\leq t_1\leq\cdots\leq t_k$라고 두고 다음 수식을 만족하게
...
(Eq. 2.1)에
의해서 $\int_{\mathbb{R}^n}p(t,x,y)dy=1$이고
$\nu_{t_1,\cdots,t_k}(F_1\times\cdots\times F_k)=\nu_{t_1,\cdots,t_k,t_{k+1},\cdots,t_{k+m}}(F_1\times\cdots\times F_k\times\mathbb{R}^n\times\cdots\times\mathbb{R}^n)$이
성립하므로 다음의 Komogorov's extension theorem을 적용하여:

\ltdiv class="env-box box-theorem">

**Theorem 2 (Kolmogorov's extension theorem).** 모든
$t_1,\cdots,t_k\in T$와 $k\in\mathbb{N}$에 대해서
$\nu_{t_1,\cdots,t_k}$를 다음 두 조건을 만족하는 probability measure on
$\mathbb{R}^{nk}$라고 하자:
...
2.  $\nu_{t_1,\cdots,t_k}(F_1\times\cdots\times F_k)=\nu_{t_1,\cdots,t_k,t_{k+1},\cdots,t_{k+m}}(F_1\times\cdots\times F_k\times\mathbb{R}^n\times\cdots\times\mathbb{R}^n)$
    for all $m\in\mathbb{N}$.

그러면 probability space $(X,\mathcal{F},P)$가 존재하고 stochastic
process $\{X_t\}$ on $X$가 존재하여, 모든 $t_i\in T$, $k\in\mathbb{N}$
and all Borel sets $F_i$에 대해 다음 조건을 만족하는
$X_t:X\to\mathbb{R}^n$이 존재한다:
...
process $\{B_t\}_{t\geq 0}$이 $X$ 위에 존재하여 다음이 만족됨을 안다:
...
\ltdiv class="env-box box-example">
...
$0\leq t_1\leq\cdots\leq t_k$에 대해서 random variable
$Z=(B_{t_1},\cdots,B_{t_k})\in\mathbb{R}^{nk}$는 (multi)normal
distribution을 가진다.
...
$\mathbb{E}^x[(B_t-x)^2]=t$,
$\mathbb{E}^x[(B_t-x)(B_s-x)]=\min(s,t)$이다.

\ltdiv class="env-box box-proof">
...
2.  $W_{t_1+t},\cdots,W_{t_k+t}$는 $t$에 의존하지 않는다. 즉,
    $\{W_t\}$는 stationary이다.
...
Brownian motion $B_t$에 대해서 다음을 정의하자:
$$B_t^{(N)} = (-N)\vee(N\wedge B_t);\quad(N=1,2,\cdots)$$ 만약 $B_t$가
continuous path를 가진다면 $t$와 $N$이 고정되어 있을 때 $s\to t$에
대해서 $|B_t^{(N)}-B_s^{(N)}|$는 $0$으로 간다. 반면에 1번 조건이
만족된다면
$$\mathbb{E}[(B_t^{(N)}-B_s^{(N)})^2]=\text{Var}(B_t^{(N)}) + \text{Var}(B_s^{(N})\geq\text{Var}(B_t^{(N)})$$
이 되어 $\text{Var}(B_t^{(N)})$는 0으로 가야 한다. 결국, 3번 조건을
이용하면 $B_t^{(N)}=0$이 되어야 한다. 그런데 이는 stochastic process라고
할 수 없다. 따라서 연속일 수 없다. ◻

\lt/div>

하지만, $W_t$를 white noise process라는 generalized stochastic process로
...
이 noise는 결론적으로 Brownian motion $B_t$ 밖에 없다는 것이 알려져
...
$$t_k=t_k^{(n)}=\begin{cases}k\cdot2^{-n}&\text{if }S\leq k\cdot2^{-n}\leq T\\S&\text{if }k\cdot2^{-n}\ltS\\T&\text{if }k\cdot2^{-n}>T\end{cases}$$
...
1.  $t^{\ast}_j=t_j$인 경우를 Itô integral이라고 부른다.

2.  $t^{\ast}_j=(t_j+t_{j+1})/2$인 경우를 Stratonovich integral이라고
    부른다.
...
$\Delta B_j=B_{t_{j+1}}-B_{t_j}$라고 두자. 그러면
...
**Proof.**
$\phi_n(s,\omega)=\sum B_j(\omega)\cdot\chi_{[t_j,t_{j+1})}(s)$로 두자.
이 때 $B_j=B_{t_j}$이다. 그러면
...
이 된다. 위에서 공부하기로 $\mathbb{E}^x[(B_t-B_s)^2]=t-s$ for
$t\geq s$였으므로,
...
함수들이고 $0\leq S\ltU\ltT$라고 하자. 그러면,

\lt/div>

1.  $\int_S^TfdB_t=\int_S^UfdB_t + \int_U^TfdB_t$이다.

2.  $\int_S^T(cf+g)dB_t=c\int_S^TfdB_t+\int_S^TgdB_t$이다.

3.  $\mathbb{E}[\int_S^TfdB_t]=0$이다.
...
**Theorem 11 (1-Dimensional Itô Formula).** $X_t$를 다음처럼 주어진 Itô
process라고 하자:
...
$g\in C^2([0,\infty)\times\mathbb{R})$이라고 하자. 그러면 $Y_t=g(t,X_t)$
또한 Itô formula이며
...
가 성립한다. 이 때 $(dX_t)^2=(dX_t)\cdot(dX_t)$는 다음의 규칙에 의해서
계산된다:
...
**Proof.** $X_t=B_t$로 두고 $g(t,x)=\frac{1}{2}x^2$으로 두면
...
**Proof.** $g(t,x)=tx$와 $X_t=B_t$로 두자. 그러면 $g(t,B_t)=tB_t$이고, Itô
formul에 의해
...
$g:\mathbb{R}\to\mathbb{R}$이 $C^2$ everywhere이라고 하고 $B_t$를
1-dimensional Brownian motion이라고 하자. 그러면 다음이 성립한다:
$$g(B_t) = g(B_0) + \int_0^t g'(B_s)dB_s + \frac{1}{2}\int_0^tg''(B_s)ds$$
이는 Brownian motion $B_t$에 대한 Taylor expansion이다(왜 그러한가?).
...
stochastic process
$X_t(\omega)=X(t,\omega):[s,\infty)\times\Omega\to\mathbb{R}^n$로 다음의
stochastic differential equation을 만족하는 것을 말한다.
$$dX_t = b(X_t)dt + \sigma(X_t)dB_t,\quad t\geq s;X_s=x$$ 이 때 $B_t$는
...
**Definition 15** (Infinitesimal Generator). $\{X_t\}$가
(time-homogeneous) Itô diffusion in $\mathbb{R}^n$이라고 하자. 그러면
(infinitesimal) generator $A$ of $X_t$는 다음처럼 정의된다:
...
**Theorem 16**. $X_t$를 다음 형태의 Itô diffusion이라고 하자.
$$dX_t = b(X_t)dt + \sigma(X_t)dB_t$$ $f$가 $C^2_0(\mathbb{R}^n)$(두 번
...
$X=\begin{pmatrix}X_1\\X_2\end{pmatrix}$을 다음 stochastic differential
equation
...
이제 $X_t$가 *density* $p_t(x,y)$를 가진다는 것을
$$\mathbb{E}^x[f(X_t)] = \int_{\mathbb{R}^n}f(y)p_t(x,y)dy$$ 를 만족하는
$p_t(x,y)$가 존재한다는 것으로 정의하면 (for every $f$) Dynkin's
formula에 의해서
...
$\text{Re}[\lambda_i(A)]\lt0$ for all $i$라고 두며 $B_t$는 standard
Brownian motion (=Wiener process)이며 $x(t)$가 미래의 $w$의 increment에
대해서는 independent이고 과거의 $w$에 대해서는 dependent라고 하자. 즉,
$t_2>t_1\geq t$라고 하면 $w(t_2)-w(t_1)$은 $x(t)$와 independent이지만
$t_3\ltt_4\leq t$에 대해서는 $w(t_3)-w(t_4)$가 dependent일 수도 있다고
하자.
...
의 꼴로 $\text{Re}[\lambda_i(\bar{A})]>0$ for all $i$이고 $\bar{B}_t$는
...
=&-BB^T\quad\quad(\because\text{Re}[\lambda_i(A)]\lt0\quad\forall i)
...
하자. 이는 $B$의 rank가 $n$이라는 말이며 동시에 $i\ltn$에 대해 $A^i$가
...
그러면 $\text{Re}[\lambda_i(A+BB^TP^{-1})]\geq0$임을 얻을 수 있고 따라서
이는 reverse-time model이 된다.

## Construction of reverse time nonlinear models

$(\Omega, \mathcal{A},P)$를 고정된 probability space라고 하고
$\{\mathcal{A}_t,-\infty\ltt\lt\infty\}$를 증가하는 sub-$\sigma$-algebra of
$\mathcal{A}$라고 하자. 그리고 $\{B_t,-\infty\ltt\lt\infty\}$를 $r$-차원
Brownian motion이라고 하고 $B_t$가 $\mathcal{A}_t$-measurable이며
$t\geq s$에 대해 $B_t-B_s$를 $\mathcal{A}_s$에 대해 independent라고
하자. 우리는 $s\geq0$에 대해
...
형태라고 가정한다:$$dx_t=f(x_t,t)dt+g(x_t,t)dB_t$$ 이 때 $x_t$는
...
위해 decreasing familyt $\{\bar{\mathcal{A}}_t,-\infty\ltt\lt\infty\}$ of
sub-$\sigma$-algebras on $\mathcal{A}$를 생각하고 $n$-vector process
$\{\bar{B}_t,-\infty\ltt\lt\infty\}$를 생각해서 $\bar{B}_t$가
...
density를 $p(x_t,t|x_s,s)$ for $t>s$라고 할 때
...
\ltdiv class="env-box box-lemma">

**Lemma 1.** $p(x_t,t)$가 위 forward Kolmogorov equation의 해라고 하자.
그리고 $$
...
**Proof.** 위 Fokker-Planck 방정식에서 $p(x_t,\overline{B}_t,t)$ 자리에
$p(x_t,t)\phi(\overline{B}_t,t)$를 대입하면
...
density $p(x_t,\overline{B}_t,t|\overline{B}_s,s)$는 다음을 만족한다:
...
하고 $p_A(a)$, etc들을 conditional probability들이라고 하자. 그러면 만약
$$p_{B|C}(b|c) = f(b-c)$$ 가 성립하는 함수 $f$가 있다면 $D=B-C$로 두었을
...
**Lemma 5.** $x_t,\overline{B}_t$를 위처럼 정의하면 $t\geq s\geq t_0$에
대해서
...
**Proof.** **Lemma 4**에
$a=x_t,b=\overline{B}_t,c=\overline{B}_s,f=\psi$를 대입하면 된다. ◻
